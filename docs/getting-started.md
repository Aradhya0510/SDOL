...
This guide covers installation, setup, and basic working examples to get you up and running with Provena.
...
Provena sits between your AI agent and its data sources. Instead of dumping raw query results into a flat context window, Provena gives your agent a **structured, provenance-enriched context frame** — so it always knows where data came from, how fresh it is, and how much to trust it.

The flow is:

```
Your Agent  →  formulate intent  →  Provena routes & executes  →  ContextFrame back to agent
```
...
Combine multiple intents into a single query. Provena decomposes the composite, routes each sub-intent to the best connector, executes them (in parallel where possible), and assembles a unified context frame.
...
After querying, Provena generates an epistemic summary you can inject into your LLM's system prompt. This tells the model about data confidence, conflicts, and uncertainty.

```python
frame = await provena.query(intent)

# Generate and inject
epistemic_context = provena.get_epistemic_context()
...
Every `provena.query()` returns a `ContextFrame`. Here's how to work with it:
